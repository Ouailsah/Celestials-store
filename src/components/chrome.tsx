...
export function Footer() {
  ...
  return <footer className="footer"><div className="footer-top"><div><span className="eyebrow">{t("THE CELESTIALS PHILOSOPHY")}</span><h2>{t("Less noise.")}<br />{t("More presence.")}</h2><p>{t("Considered essentials. Independent spirit.")}<br />{t("For the ones who move differently.")}</p></div><div className="footer-links"><div><span className="eyebrow">{t("EXPLORE")}</span><Link href="/shop">{t("All pieces")}</Link><Link href="/shop?edit=new">{t("New arrivals")}</Link><Link href="/about">{t("Our story")}</Link></div><div><span className="eyebrow">{t("HERE TO HELP")}</span><Link href="/contact">{t("Contact us")}</Link><Link href="/contact#delivery">{t("Delivery & returns")}</Link><Link href="/contact#sizing">{t("Size guide")}</Link><Link href="/admin">{t("Admin access")}</Link></div></div></div><div className="footer-wordmark">CELESTIALS<BrandMark className="footer-brand-mark" /></div><div className="footer-bottom"><span>© {new Date().getFullYear()}{t(" CELESTIALS. ALL RIGHTS RESERVED.")}</span><span>{t("DESIGNED TO GO YOUR OWN WAY.")}</span><Link href="/contact" aria-label={t("Connect with CELESTIALS")}><ArrowUpRight size={16} /></Link></div></footer>;
}
